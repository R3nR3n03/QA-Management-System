...
`.checks-screen` is the third screen to take the 1440px opt-in, and spends it the same way:
`.detail-cols` puts the upload in the wide column with the facts that qualify it in the aside,
and the batch table spans the full width underneath — a table of files, people and tallies is
the one thing on that screen that can use every pixel it is given. Inside the 1040px reading
measure the same two columns left the upload form about 650px with a 340px aside pressed
against it, and the form is the work.

That table now literally spends the width: nine columns, because each outcome is its own
`.num` column rather than a chip in one shared cell. **Their order is a constraint, not a
...
about 970px of the ~1150px available, so nothing clips today; the ordering is what keeps that
true without anyone re-measuring it.
